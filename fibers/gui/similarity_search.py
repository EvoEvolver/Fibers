...
def prepare_similarity_grid(src_lists, weight_list, query, content_list, width=5):
    ...
    for i, index in enumerate(index_start):
        similarity_for_node = src_similarities[
                              index:index + min(len(src_lists[i]), width)]
        matched_query = []
        matched_query_score = []
        for j, similarity in enumerate(similarity_for_node):
        ...
        for j in range(len(matched_query_list[i])):
            if matched_query_list[i][j] == "":
                ...
                text += "query: " + matched_query_list[i][j] + "<br>" + "score: " + str(
                    round(new_score, 3))
                display_text_for_node.append(text)
                if score is not None:
                    matched_query_score_list[i][j] = new_score
        display_texts.append(display_text_for_node)
